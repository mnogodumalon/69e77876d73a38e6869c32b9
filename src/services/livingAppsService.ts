...
import type { Speisekarte, Bestellrunde, Bestellposition, CreateSpeisekarte, CreateBestellrunde, CreateBestellposition } from '@/types/app';
...
export class LivingAppsService {
  // --- SPEISEKARTE ---
  static async getSpeisekarte(): Promise<Speisekarte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SPEISEKARTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Speisekarte[];
    return enrichLookupFields(records, 'speisekarte');
  }
  static async getSpeisekarteEntry(id: string): Promise<Speisekarte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SPEISEKARTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Speisekarte;
    return enrichLookupFields([record], 'speisekarte')[0];
  }
  static async createSpeisekarteEntry(fields: CreateSpeisekarte) {
    return callApi('POST', `/apps/${APP_IDS.SPEISEKARTE}/records`, { fields: cleanFieldsForApi(fields as any, 'speisekarte') });
  }
  static async updateSpeisekarteEntry(id: string, fields: Partial<CreateSpeisekarte>) {
    return callApi('PATCH', `/apps/${APP_IDS.SPEISEKARTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'speisekarte') });
  }
  static async deleteSpeisekarteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SPEISEKARTE}/records/${id}`);
  }
  ...
  // --- BESTELLPOSITION ---
  static async getBestellposition(): Promise<Bestellposition[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BESTELLPOSITION}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Bestellposition[];
    return enrichLookupFields(records, 'bestellposition');
  }
  static async getBestellpositionEntry(id: string): Promise<Bestellposition | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BESTELLPOSITION}/records/${id}`);
    const record = { record_id: data.id, ...data } as Bestellposition;
    return enrichLookupFields([record], 'bestellposition')[0];
  }
  static async createBestellpositionEntry(fields: CreateBestellposition) {
    return callApi('POST', `/apps/${APP_IDS.BESTELLPOSITION}/records`, { fields: cleanFieldsForApi(fields as any, 'bestellposition') });
  }
  static async updateBestellpositionEntry(id: string, fields: Partial<CreateBestellposition>) {
    return callApi('PATCH', `/apps/${APP_IDS.BESTELLPOSITION}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bestellposition') });
  }
  static async deleteBestellpositionEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BESTELLPOSITION}/records/${id}`);
  }
