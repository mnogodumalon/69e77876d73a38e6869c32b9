...
const SPEISEKARTE_FIELDS = [
  { key: 'gericht_name', label: 'Gerichtname', type: 'string/text' },
  { key: 'gericht_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
...
const BESTELLRUNDE_FIELDS = [
  ...
  { key: 'bestellfrist', label: 'Bestellfrist', type: 'date/datetimeminute' },
  { key: 'lieferzeit', label: 'Voraussichtliche Lieferzeit', type: 'date/datetimeminute' },
  { key: 'mindestbestellwert', label: 'Mindestbestellwert (€)', type: 'number' },
  { key: 'lieferkosten', label: 'Lieferkosten (€)', type: 'number' },
  { key: 'runde_hinweis', label: 'Hinweis / Anmerkung', type: 'string/textarea' },
  { key: 'organisator_vorname', label: 'Vorname des Organisators', type: 'string/text' },
...
export default function AdminPage() {
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'bestellposition':
        ...
        lists.speisekarteList = (data as any).speisekarte ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'bestellposition' && fieldKey === 'ausgewaehlte_runde') {
      const match = (lists.bestellrundeList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.runde_titel ?? '—';
    }
    if (entity === 'bestellposition' && fieldKey === 'ausgewaehltes_gericht') {
      const match = (lists.speisekarteList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.gericht_name ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'speisekarte': return {
        create: (fields: any) => LivingAppsService.createSpeisekarteEntry(fields),
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'speisekarte' || dialogState?.entity === 'speisekarte') && (
        <SpeisekarteDialog
          open={createEntity === 'speisekarte' || dialogState?.entity === 'speisekarte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'speisekarte' ? handleUpdate : (fields: any) => handleCreate('speisekarte', fields)}
          defaultValues={dialogState?.entity === 'speisekarte' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'bestellposition' || dialogState?.entity === 'bestellposition') && (
        <BestellpositionDialog
          ...
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bestellposition' ? handleUpdate : (fields: any) => handleCreate('bestellposition', fields)}
          defaultValues={dialogState?.entity === 'bestellposition' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'bestellposition' && (
        <BestellpositionViewDialog
          open={viewState?.entity === 'bestellposition'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
